  ...
  angular
    ...
    .provider('Spotify', function () {
      ...
      this.$get = ['$q', '$http', '$window', function ($q, $http, $window) {
        ...
        NgSpotify.prototype = {
          api: function (endpoint, method, params, data, headers, refresh) {
            var deferred = $q.defer();

            var apiBase = this.apiBase;

            if(refresh) {
              apiBase = "/lineup/spotify";
            }
          ...
          removePlaylistTracks: function (userId, playlistId, tracks) {
            ...
            angular.forEach(tracks, function (value, index) {
              ...
              if(typeof track === "string") {
                ...
                tracks[index] = {
                  uri: id.indexOf('spotify:') === -1 ? 'spotify:track:' + id : id,
                  positions: positions
                };
              }
          ...
          refreshToken: function(token) {
            ...
            return this.api('/api/token', 'POST', null, data, this._auth(), true);
          }
        };
